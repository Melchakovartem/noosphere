...
contract('Crowdsale', function(accounts) {
    ...
    function get_purchased_tokens(eth) {
    	return eth / 0.003785; //solution for rounding problems    
    }
    ...
    it('returns remaining money when hard cap is reached', async function(){
        eth_invest1 = ETH(251.215);
        eth_invest2 = ETH(4.785);
        await crowdsale.setTime(startTime + 10, {from: owner});
        balanceOwner = Number(web3.eth.getBalance(owner));

        purchased_tokens2 = get_purchased_tokens(ETH(3.785));
        totalTokens2 = purchased_tokens2;

        
        await crowdsale.sendTransaction({from: investor1, to: crowdsale_address, value: eth_invest1});
        await crowdsale.sendTransaction({from: investor2, to: crowdsale_address, value: eth_invest2});
        
        assert.equal(await token.balanceOf(investor2), totalTokens2);
        assert.equal(await crowdsale.totalCollected(), ETH(255));

        totalBalanceOwner = balanceOwner + hardCap;
        assert.equal(web3.eth.getBalance(owner), totalBalanceOwner);
    })
